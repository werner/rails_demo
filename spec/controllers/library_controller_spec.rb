...
RSpec.describe LibraryController, type: :controller do
  ...
  describe 'GET #library.json' do
    ...
    it 'get the content ordered by purchase date' do
      dates = JSON.parse(response.body)['library'].map do |l|
        l['purchasable']['created_at'].to_time
      end
      # this splits and compares the dates to see if they are ordered,
      # it's using the spaceship operator to compare the dates
